...
# filepath =  os.path.join(component,'registry.n3')

# print(filepath)

# print(Graph().parse(filepath))
# input('waiting')
...
def OperatingTask (task_name,input_file_path,desir_output_type,task_domain,namespace,purpose):
    ...
    if len(task_name) > 0 and len(desir_output_type)> 0:
        ...
        g.serialize(destination=component+'contextkg'+".n3")
        g.close()
        ctx=''
        if openflag ==1:
...
def commonKGlearning(datasetout,task_id,outputv,ns):
    ...
    for ik in k[1]:
        ...
        i=i+1
    kg = Graph()
    kg.parse(component+'taskinput.n3')
    #subject
    task = URIRef(namespace+'/'+task_id)
    #verb
    ...
    if len(listOfms)>0:
        ...
        if len(task_id):
            ...
            for ms in listOfms:
                ...
                i=i+1
        kg.serialize(destination=componenet+'workflows'+".n3")
        kg.close()
...
def savemodel(task_id,model,ns):
    ...
    kg = Graph()
    kg.parse(component+"solution.n3")
    #subject
    task = URIRef(namespace+'/'+task_id)
    #verb
